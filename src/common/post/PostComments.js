            ...
            {comments.map(comment => (
                <Box key={comment.id} display="flex" alignItems="center" justifyContent="flex-start">
                    <Typography variant="body2"
                                className={classes.textStrong + " " + classes.dense}>{props.postUser}: </Typography>
                    <Typography variant="body2"
                                className={classes.textLite + " " + classes.dense}>{comment.text}</Typography>
                </Box>
            ))}
            {props.children}
            <FormControl className={classes.container + " " + classes.dense} variant="standard">
                <InputLabel className={classes.dense} htmlFor={props.baseId + "-field-comment"}>Add a
                    comment</InputLabel>
                <Input className={classes.textInput + " " + classes.dense} id={props.baseId + "-field-comment"}
                       type="text" value={text} onChange={onInputChange}/>
                <Button className={classes.btnAdd} variant="contained" color="primary" id="btn-add"
